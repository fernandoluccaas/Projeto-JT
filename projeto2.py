...
ARQUIVO_RELATORIO_PREVISOES = 'relatorio_previsoes.xlsx' # Novo arquivo
...
def carregar_ou_iniciar_estoque(data_inicial, previsoes_df):
    if os.path.exists(ARQUIVO_ESTADO_ESTOQUE):
        ...
        return pd.read_csv(ARQUIVO_ESTADO_ESTOQUE, parse_dates=['data_atual']).iloc[0]
    else:
        ...
        previsao_amanha = previsoes_df.loc[previsoes_df['ds'] == data_inicial + timedelta(days=1), 'yhat'].iloc[0] if not previsoes_df.loc[previsoes_df['ds'] == data_inicial + timedelta(days=1)].empty else 0
        return pd.Series({
            'data_atual': data_inicial, 'kg_descongelando_d1': 0.0,
            'kg_descongelando_d2': max(0, previsao_amanha), 'kg_pronto_venda_dia1': max(0, previsao_hoje),
            'kg_pronto_venda_dia2': 0.0,
        })

def gerar_relatorio_previsoes(forecast_df, data_inicio):
    """Gera um relatório separado contendo apenas as previsões puras do Prophet."""
    ...
    for data_base in previsoes_futuras['ds']:
        data_d1 = data_base + timedelta(days=1)
        data_d2 = data_base + timedelta(days=2)
        ...
        if data_d2 in previsoes_futuras['ds'].values:
            ...
            lista_relatorio_previsao.append({
                'Data da Retirada': data_base.strftime('%d/%m/%Y'),
                'SKU (frango)': SKU_PRODUTO,
...
def executar_rodada_diaria(estado_atual, previsoes_df):
    ...
    relatorio_df = pd.DataFrame([{'Data da Retirada': hoje.strftime('%d/%m/%Y'), 'SKU (frango)': SKU_PRODUTO,
        ...
        'Kg em Descongelamento (pronto amanhã)': f"{estado_atual['kg_descongelando_d2']:.2f} kg",
    ...
    estado_amanha = {'data_atual': hoje + timedelta(days=1), 'kg_descongelando_d1': kg_a_descongelar_hoje,
        'kg_descongelando_d2': estado_atual['kg_descongelando_d1'], 'kg_pronto_venda_dia1': estado_atual['kg_descongelando_d2'],
...
if __name__ == "__main__":
    ...
    if acao == '2': resetar_estado()
        ...
        # Chamada para a nova função que gera o relatório de previsões
        ...
        pd.DataFrame([proximo_estado]).to_csv(ARQUIVO_ESTADO_ESTOQUE, index=False)
        print(f"\nEstado para amanhã salvo com sucesso.")
